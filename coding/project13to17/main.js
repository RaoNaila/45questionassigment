...
console.log("unfortunatly! the new dinner table will not arrive so we can invite only two guest.");
//removing 4 guest only 2 guest remain and each time pop a message for
// uninviting to removing guest.
while (guestList.length > 2) {
    let removedGuest = guestList.pop();
    if (removedGuest !== undefined) {
        console.log(`sorry ${removedGuest}, we can not invite you because of unsufficient arrangement.`);
    }
}
